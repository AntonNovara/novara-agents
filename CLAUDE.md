...
**Wann es ein PDF gibt (25.09.2026, `agents/field_worker_agent.py`).** Ein Regiebericht-PDF entsteht NUR bei echter Arbeitsinformation mit allen Pflichtangaben (Tätigkeit, Kunde/Baustelle, Stunden > 0). Graph: `extract_entities` (LLM liefert u. a. `language` als ISO-Code und `is_work_report`) → `evaluate` (deterministisch im Code: `ready_for_pdf`, `missing_fields`, `guidance_type` = `greeting` | `missing` | `retry`) → bei "kein PDF" `compose_guidance` (zweiter, kurzer LLM-Aufruf: freundlicher Text in der Sprache des Technikers, bei Fehlern Vorlagen de/es/en) → `finalize`. Grüße, Smalltalk, Fragen, unvollständige Angaben und Extraktionsfehler bekommen ausschließlich `data["reply"]`; `main.py` prüft `ready_for_pdf is False` und sendet dann nur diesen Text (`[STEP 3b]` im Log), kein PDF, kein `Leads_Demo`-Eintrag. Wichtig: bei erfolgreicher Extraktion wird ein leeres `arbeit` NICHT mit dem Rohtext aufgefüllt (sonst zählte "hola" als Arbeit); der Rohtext-Fallback gilt nur noch für die Extraktion selbst und führt zu `retry`. Getestet mit dem echten Modell in de/es/en/tr (Begrüßung, unvollständig, vollständig) und in TEST 24/26.

**Härtung nach Code-Review (25.09.2026, TEST 29).** Ein unabhängiger Review fand (alle verifiziert, keine kritischen): (1) LLM-Felder nicht typsicher -- `stunden=NaN/9999` erzeugte ein PDF, ein Zahl-/Listenwert bei `kunde` warf `AttributeError` → jetzt `_as_text()`/`_as_hours()` (Stunden nur endlich und in (0, 24], sonst Nachfrage); (2) `ready_for_pdf is False` → `is not True` (fail-closed); (3) Kosten-/Missbrauchsschutz: Absender-Limit 30 Nachrichten/Stunde (danach still verworfen, ein Hinweis pro Fenster), Sprachnachrichten max. 180 s, Media max. 5 MB und nur https; (4) das lokale PDF wird IMMER gelöscht (`finally`) -- es lag sonst unter `/static/reports/` öffentlich abrufbar; (5) PDF-Lesefehler/Upload-Fehler → der Text geht trotzdem raus; (6) `verify_signature()` mit Nicht-ASCII-Header → `False` statt 500, und auf Railway (`RAILWAY_ENVIRONMENT_NAME`) wird ohne App Secret nie durchgelassen, auch wenn `ENVIRONMENT` fehlt (dito `dev-secret`-Bypass); (7) Sequenz-Endpoints: negativer `step_index` → 404, gescheiterter Digest → 502 (Workflow wird rot). Bewusst NICHT umgesetzt: Absender-Allowlist (Produktentscheidung, `[DEMO]` soll von jeder Nummer gehen). Offene Restpunkte (gering): Dedup nur In-Memory, Telefonnummern in Logs, `Leads_Demo` teilt sich das Spreadsheet mit dem CRM.
...
## Änderungen 25.09.2026 — Prospect-Audit + Outbound-Guard

- **`tools/prospect_audit.py`** (neu): deterministischer Website-Check für Elektriker-Prospects (12 gewichtete HTML/HTTP-Checks: WhatsApp-Link, Viewport, tel:-Link, Formular, Ladezeit, HTTPS, Title/Description, Impressum, LocalBusiness-Schema, Notdienst, Karte). Kein LLM, kein Apify/Firecrawl. SSRF-geschützt (nur http/https, nur öffentliche IPs, auch nach Redirects; 1,5 MB / 8 s Limit). Ergebnis in Tabelle `prospect_audits`. Endpoints: `POST /api/v1/tools/prospect-audit`, `GET /api/v1/tools/prospect-audit/{id}` (API-Key). Bericht (`render_report_de`) erfindet keine Verlust-Zahlen; die tatsächliche Antwortzeit wird als "nicht geprüft" ausgewiesen. **Grenze:** rein HTML-basiert — per JavaScript nachgeladene Inhalte (z. B. ein WhatsApp-Button in einem JS-Widget) werden nicht gesehen, ein niedriger Score ist daher ein Hinweis, kein Beweis.
- **`core/outbound_guard.py`** (neu): harte Regeln für jede SDR-Erstnachricht — nur Preise aus `novara_wissen.txt`, keine Platzhalter/Demo-Texte, keine Garantieversprechen, kein LLM-Artefakt, Opt-out-Hinweis Pflicht. `SDRGraph.compose_outreach()` ergänzt den Opt-out deterministisch und ersetzt bei Verstoß den LLM-Text durch eine sichere Vorlage; Verstöße stehen in `final_result.outreach.guard_violations`. (`GuardianAgent` ist ein Infrastruktur-Auditor und prüft KEINE Nachrichten.)
- Tests: `test_prospect_audit()`, `test_outbound_guard()` — 188 PASS.

## LLM-Provider-Selektor (25.09.2026)

`LLM_PROVIDER=anthropic|ollama` (Default `anthropic`, Verhalten unverändert). Bei `ollama` liefert `core.llm.build_llm()` einen `_OllamaChatModel` (REST `/api/chat`, `OLLAMA_BASE_URL`, `OLLAMA_MODEL`), erzwingt `format: json` bei JSON-Prompts und braucht keinen Anthropic-Key. Keine Bild-Inputs (document_node fängt das ab). Railway erreicht ein lokales Ollama nicht -- Produktion bleibt auf `anthropic`, bis ein erreichbarer, abgesicherter Ollama-Host existiert. Der Outbound-Guard gilt provider-unabhängig. Test: `test_llm_provider()` (Ollama gemockt, 197 PASS).

**Ollama-Evaluation SDR (25.09.2026, 5 Outbound-Fälle, `SDRGraph` komplett, CRM gemockt):** `qwen2.5:7b` und `hermes3:8b` (Nous Research) erfüllen die Anforderungen an Erstnachrichten NICHT. Qwen: 52–60 s/Lead, klebt Kalt-Mail + Follow-ups zusammen, Markdown, `[Datum]`, erfindet ein "Kassen-System"-Angebot für eine Bäckerei. Hermes: 19–41 s/Lead, sauberer (2 von 5 saubere Erstmails), aber Tippfehler ("Halo"), erfundenes "KI-gestütztes Kassensystem" für die Bäckerei (vom Guard NICHT erkannt), und beide Modelle bewerten die Bäckerei mit ICP 60–75 (`score_lead`-Schwelle 40 lässt sie durch — Schwelle/Rubrik separat prüfen, nicht nur Modellproblem). **Entscheidung: SDR/Chat bleiben auf Anthropic (`LLM_PROVIDER=anthropic`); Ollama nur für interne, risikoarme Aufgaben** (Klassifikation, Extraktion, vertrauliche Texte). Ein Vergleich gegen Claude steht aus, solange der neue Anthropic-Key keinen Workspace hat.

**SDR-Qualifizierung + Angebots-Guard (25.09.2026):** (1) `SDRGraph.score_lead()` qualifiziert im OUTBOUND nur noch bei `icp_score >= OUTBOUND_MIN_ICP` (70, reiner Branchen-Fit OHNE Seniority-Bonus) UND Gesamtscore >= 40. Vorher ließ die Schwelle 40 jede KMU durch (Rubrik "MITTEL 45-69") und der Seniority-Bonus konnte einen falschen Sektor retten -- eine Bäckerei bekam einen Pitch für ein erfundenes Kassensystem. Der Inbound-Chat behält `QUALIFICATION_THRESHOLD` (40): dort hat der Besucher uns selbst kontaktiert. Folge: Nicht-Handwerks-KMU (ICP 45-69) werden im Outbound disqualifiziert -- bei Bedarf manuell nachfassen. (2) `core/outbound_guard.py` prüft Angebots-Formulierungen ("unser/unsere … System/Software/Lösung/App/Tool/Plattform/Assistent/Bot"): jedes Substantiv darin muss im Vokabular von `novara_wissen.txt` vorkommen, sonst Verstoß -> sichere Vorlage. Bewusst grob (Wortvokabular, keine Semantik): erkennt erfundene Produkte wie "Kassen-System", nicht jede falsche Behauptung.

**SDR ↔ Prospect-Audit (25.09.2026):** `SDRGraph` hat den neuen Node `audit_prospect` (zwischen `check_consent` und `compose_outreach`, also NUR für qualifizierte, einwilligungsfähige Leads). Die Website wird per Regex aus dem Eingabetext gezogen (`_find_website`; E-Mail-Domains zählen nicht), `tools.prospect_audit.run_audit()` ruft sie ab (SSRF-geschützt, gespeichert in `prospect_audits`). Die 2 größten Lücken gehen als Faktenblock in den Outreach-Prompt (`_audit_context`), mit Anweisung: höchstens eine Lücke, vorsichtig formuliert ("ich konnte … nicht finden"), keine Verlustzahlen — der Check liest nur HTML, ohne JavaScript (z. B. lädt unsere eigene Seite ihren Kontakt über ein JS-Chat-Widget, kein `wa.me`-Link). Nie blockierend: Fehler/kein Website-Fund → Outreach wie bisher, `final_result.website_audit` bleibt leer bzw. trägt den Fehler. Tests: `test_sdr_prospect_audit_integration()` (204 PASS); zusätzlich einmal live mit Claude + echtem Audit geprüft.

**Erfundener Kontaktname im CRM (26.09.2026):** Bei `contact_source == "generated"` (Ansprechpartner vom LLM geraten, Firma nicht in der Lead-DB) speichern `write_to_crm` (`LeadRecord.contact_name`, damit auch `customer_state`) und `final_result.contact.name` den Namen `Unbekannt` (`UNKNOWN_CONTACT_NAME`, `_contact_display_name()`), nie den erfundenen. Echte DB-Kontakte behalten ihren Namen. **Offen:** E-Mail und LinkedIn-URL des erfundenen Kontakts sind ebenfalls geraten (`vorname.nachname@firma.at`) und werden weiterhin gespeichert und als Identifier für Consent/Sequenz genutzt. Test: `test_sdr_generated_contact_crm()` (205 PASS).
